...
class _FullHistoryPageState extends State<FullHistoryPage>
  ...
  Future<void> _loadFullHistory() async {
    ...
    if (_history.isEmpty) {
      // Если история пуста, всегда ставим isLoading в true перед загрузкой
  ...
  void _shareHistoryItem(
      BuildContext tileContext, CalculationHistoryItem item) {
    final l10n = AppLocalizations.of(context)!;
    final DateFormat dateFormat =
        DateFormat('dd.MM.yyyy HH:mm', l10n.localeName);

    final String textToShare =
        "${l10n.calculationDetails(item.inputSideName, item.inputValue.toStringAsFixed(2), item.sideA.toStringAsFixed(2), item.sideB.toStringAsFixed(2), item.sideC.toStringAsFixed(2), item.area.toStringAsFixed(2), item.perimeter.toStringAsFixed(2))}\n${l10n.timeLabel}: ${dateFormat.format(item.timestamp.toLocal())}";
            ...
            child: Column(
              ...
              children: [
                Icon(Icons.history_toggle_off_outlined,
                    size: 60, color: theme.hintColor.withOpacity(0.7)),
      ...
      pageContent = RefreshIndicator(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            // Используем context из itemBuilder
            final item = _history[index];
            return Card(
              child: Builder(
                  // Обертка для ListTile, чтобы получить его контекст для Share
          ...
          Padding(
            ...
            child: Align(
              ...
              child: TextButton.icon(
                ...
                label: Text(l10n.clearHistoryButton,
